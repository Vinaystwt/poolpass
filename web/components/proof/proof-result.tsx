    ...
    <div className="flex flex-col gap-lg">
      ...
      <div>
        <p className="mb-xs text-micro-cap uppercase tracking-wide text-ink-mute">
          Public inputs · [merkle_root, amount, nullifier, epoch]
        </p>
        <dl className="divide-y divide-hairline rounded-lg border border-hairline">
          <Row label="merkle_root" value={<HashChip value={decoded.merkle_root} />} />
          <Row
            label="amount"
            value={
          ...
          />
          <Row label="nullifier" value={<HashChip value={decoded.nullifier} />} />
          <Row label="epoch" value={<span className="tnum text-body-md text-ink">{decoded.epoch}</span>} />
        </dl>
      </div>
